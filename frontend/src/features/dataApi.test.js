import assert from 'node:assert/strict';
import test from 'node:test';
import api from '../services/api.js';
import { deleteCurrentAccount, getCurrentUser } from './auth/auth.api.js';
import { getDashboardStats, getRecentInterviews } from './dashboard/dashboard.api.js';
import { deleteInterviewReport, getInterviewReports } from './interview/interview.api.js';
import { decodeBlobError } from './resume/resume.api.js';

test('data APIs send pagination, statistics and destructive requests to intended endpoints', async t => {
  const requests = [];
  const originalAdapter = api.defaults.adapter;
  api.defaults.adapter = async config => {
    requests.push({ url: config.url, method: config.method, params: config.params, data: config.data, signal: config.signal });
    return { data: {}, status: 200, statusText: 'OK', headers: {}, config };
  };
  t.after(() => { api.defaults.adapter = originalAdapter; });

  await getInterviewReports({ page: 3, limit: 10 });
  await getDashboardStats();
  await getRecentInterviews();
  const controller = new AbortController();
  await getCurrentUser({ signal: controller.signal });
  await deleteInterviewReport('507f1f77bcf86cd799439013');
  await deleteCurrentAccount(' password ');

  assert.deepEqual(requests, [
    { url: '/interview/', method: 'get', params: { page: 3, limit: 10 }, data: undefined, signal: undefined },
    { url: '/interview/stats', method: 'get', params: undefined, data: undefined, signal: undefined },
    { url: '/interview/', method: 'get', params: { page: 1, limit: 5 }, data: undefined, signal: undefined },
    { url: '/auth/get-me', method: 'get', params: undefined, data: undefined, signal: controller.signal },
    { url: '/interview/report/507f1f77bcf86cd799439013', method: 'delete', params: undefined, data: undefined, signal: undefined },
    { url: '/auth/account', method: 'delete', params: undefined, data: JSON.stringify({ password: ' password ' }), signal: undefined }
  ]);
});

test('PDF blob errors expose safe backend JSON messages', async () => {
  const error = {
    response: {
      status: 502,
      data: new Blob(
        [JSON.stringify({ message: 'Unable to complete AI generation. Please try again later.' })],
        { type: 'application/json; charset=utf-8' }
      ),
    },
  };
  assert.equal(
    (await decodeBlobError(error)).response.data.message,
    'Unable to complete AI generation. Please try again later.'
  );
});
